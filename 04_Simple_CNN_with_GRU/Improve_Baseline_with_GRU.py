...
###############################################
#########################################################
### Simple baseline model with 2 additional GRU units ###
#########################################################
def create_standard_CNN(name='standard', lr=0.01):
    ...
    model = Sequential()
    filters = (5, 5)
    # FIXME: tirei os nomes das layers - name='disc_1'
    ...
    model.add(Conv2D(16, filters, strides=(1, 1), activation='relu', data_format='channels_last',
                          kernel_initializer='he_normal', input_shape=(width_time, 128, 1)))
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))  # default poolsize = (2, 2), stride = (2,2)
    ...
    # model_k2c2.add(Conv2D(25, (5, 5), strides=(1, 1), activation='relu', data_format='channels_last', kernel_initializer='he_normal'))
    model.add(Dropout(0.1))
    model.add(Conv2D(16, filters, strides=(1, 1), activation='relu', data_format='channels_last',
                          kernel_initializer='he_normal'))
    model.add(MaxPooling2D(pool_size=(3, 2), padding='same'))  # default poolsize = (2, 2), stride = (2,2)
    model.add(BatchNormalization())
    model.add(Dropout(0.1))
    model.add(Conv2D(16, filters, strides=(1, 1), activation='relu', data_format='channels_last',
                          kernel_initializer='he_normal'))
    ...
    model.add(Reshape(target_shape=(1, 2000)))
    model.add(GRU(2000, return_sequences=True))
    model.add(GRU(2000))
    ...
    # model_k2c2.load_weights('GAN_weights_leakyRelu_kernel_5.h5', by_name=True)

    # print(model_k2c2.summary())
    return model
...
def run_f1score(X, labels, epochs, batch_size, learning_rate, resume_training=False):
    ...
    for train, test in tqdm(folds):
        ...
        history = model.fit(x=X[train], y=labels[train],
    ...
    for bs in range(len(matrix_of_params[0])):
        for ep in range(len(matrix_of_params[1])):
            for lr in range(len(matrix_of_params[2])):
                ...
                file_name = 'history_improve_gru_bs_' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1s_name = 'f1_score_improve_gru_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1e_name = 'f1_each_improve_gru_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                conf_name = 'conf_mat_improve_gru_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                scores_name = 'scores_improve_gru_bs' + str(matrix_of_params[0][bs]) + \
                              '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                time_name = 'time_improve_gru_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
