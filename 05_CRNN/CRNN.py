...
from keras.layers import Input, Dense, Dropout, BatchNormalization, Input, Reshape, GRU
...
from Conv2dRegular import Conv2dRegular
...
###############################################
#########################################################################################
### CRNN model, implemented in PyTorch by Won et al. #################################
### https://github.com/minzwon/sota-music-tagging-models/blob/master/training/model.py ##
#########################################################################################
def compile_crnn(name='crnn_with_gru', lr=0.01):
    width_time = 942
    num_classes = 4

    model = Sequential()

    model.add(Input(shape=(width_time, 128, 1)))

    model.add(BatchNormalization())

    model.add(Conv2dRegular(64, pooling=(2, 2)))
    model.add(Conv2dRegular(128, pooling=(3, 3)))
    model.add(Conv2dRegular(128, pooling=(4, 4)))
    model.add(Conv2dRegular(128, pooling=(4, 4)))

    model.add(Reshape((9, 128)))
    model.add(GRU(32, name='gru_0', return_sequences=True))
    model.add(GRU(32, name='gru_1'))
...
def run_f1score(X, labels, epochs, batch_size, learning_rate, resume_training=False):
    ...
    for train, test in tqdm(folds):
        ...
        model = compile_crnn(lr=learning_rate)
    ...
    for bs in range(len(matrix_of_params[0])):
        for ep in range(len(matrix_of_params[1])):
            for lr in range(len(matrix_of_params[2])):
                ...
                file_name = 'history_crnn_bs_' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1s_name = 'f1_score_crnn_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1e_name = 'f1_each_crnn_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                conf_name = 'conf_mat_crnn_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                scores_name = 'scores_crnn_bs' + str(matrix_of_params[0][bs]) + \
                              '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                time_name = 'time_crnn_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
