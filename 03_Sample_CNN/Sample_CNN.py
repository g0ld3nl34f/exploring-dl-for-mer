...
import History
from Residual1D import Residual1D
...
def compile_model(learning_rate=0.0001):
    ...
    # Defaults Residual1D_SE layer
    # kernel_size=3, stride=1, pool_size=3, diff_size=False
    model.add(Input(shape=(width, 1)))
    model.add(Residual1D(filters=num_filters, stride=3, pool_size=1))
    model.add(Residual1D(filters=num_filters, pool_size=3))
    model.add(Residual1D(filters=num_filters, pool_size=3))
    model.add(Residual1D(filters=num_filters * 2, pool_size=3))
    model.add(Residual1D(filters=num_filters * 2, pool_size=3))
    model.add(Residual1D(filters=num_filters * 2, pool_size=3))
    model.add(Residual1D(filters=num_filters * 2, pool_size=3))
    model.add(Residual1D(filters=num_filters * 2, pool_size=3))
    model.add(Residual1D(filters=num_filters * 2, pool_size=3))
    model.add(Residual1D(filters=num_filters * 4, pool_size=3))
    model.add(Residual1D(filters=num_filters * 4,
                            kernel_size=1, stride=1, pool_size=1,
                            diff_size=False))
    model.add(Dropout(rate=dropout_rate))
    model.add(Dense(units=num_class, activation='softmax'))
    model.add(Reshape(target_shape=[4]))
    ...
    for train, test in tqdm(folds):
        ...
        for test_sample in x_test:
            ...
            for samp in test_sample:
                real_test_sample.append(samp)
            real_test_sample = np.asarray(real_test_sample)
            # Predict each chunk of the full song
            ypred = model.predict_on_batch(x=real_test_sample).reshape(len(test_sample), 4)
            # Save the corresponding quad of each chunk
            ypred_quad = np.asarray(ypred).argmax(axis=1)
            y_pred_total.append(ypred_quad)
            # Get the mean of the song
