...
import History
from Generator_4QAED_audiomentations import Generator_4QAED
...
# Audio augmentations techniques to be applied to the current dataset to obtain
# new samples for training the model.
transforms = ['time_freq_masked',
                            'tanh_dist',
                            'seven_banded',
                            'random_gained',
                            'backgrounded']
trans_names = 'tf_td_sb_rg_bg'
...
# Create Mel-spectrom with padding up to the specified width
def to_melspectrogram(sample, width=942):
    mel = librosa.power_to_db(librosa.feature.melspectrogram(y=sample))
    return np.rot90(np.pad(mel, ((0, 0), (0, width-len(mel[0])))))

# Create Mel-spectrogram dataset with specified name
def create_melspect_dataset(waveform_dataset, name):
    max_len = 482000

    with open(waveform_dataset + '.pickle', 'rb') as wf:
        waveforms = pickle.load(wf)

    for count, sound in enumerate(waveforms):
        if len(sound) > max_len:
            to_cut = len(sound) - max_len
            waveforms[count] = sound[to_cut:]

    print(np.array(waveforms).shape)

    melspects = np.array([to_melspectrogram(samp) for samp in waveforms])
    with open(name + '.pickle', 'wb') as new_melspect_dataset:
        pickle.dump(melspects, new_melspect_dataset)
...
# Function for data augmentations
def augmentate_data(samples, sr=16000):
    time_mask = am.TimeMask(p=1)
    frequency_mask = am.BandStopFilter(p=1)
    seven_band = am.SevenBandParametricEQ(p=1)
    tanh_dist = am.TanhDistortion(p=1)
    time_freq_masks = am.Compose([
        time_mask,
        frequency_mask
    ])
    gain_gain = am.Gain(p=1)
    background = am.AddBackgroundNoise('back_noises/', p=1)


    time_freq_masked = []
    seven_banded, tanh_disted = [], []
    random_gained, backgrounded = [], []

    for samp in tqdm(samples.tolist()):
        time_freq_masked.append(time_freq_masks(samp, sample_rate=sr))
        seven_banded.append(seven_band(samp, sample_rate=sr))
        tanh_disted.append(tanh_dist(samp, sample_rate=sr))
        random_gained.append(gain_gain(samp, sample_rate=sr))
        backgrounded.append(background(samp, sample_rate=sr))

    with open('time_freq_masked_samples_16kHz.pickle', 'wb') as tfm_file:
        pickle.dump(time_freq_masked, tfm_file)
    with open('seven_banded_samples_16kHz.pickle', 'wb') as sb_file:
        pickle.dump(seven_banded, sb_file)
    with open('tanh_disted_samples_16kHz.pickle', 'wb') as td_file:
        pickle.dump(tanh_disted, td_file)
    with open('random_gained_samples_16kHz.pickle', 'wb') as gg_file:
        pickle.dump(random_gained, gg_file)
    with open('backgrounded_samples_16kHz.pickle', 'wb') as bg_file:
        pickle.dump(backgrounded, bg_file)

    print('Augmented samples!')


def back_to_wav(orig_path, def_path, sr=16000):
    ##
    #   orig_ds -> df with shape (samples, (song_code, waveform, target))
    #   def_ds -> df with shape(samples, waveform)
    ##

    with open(orig_path + '.pickle', 'rb') as orig_file:
        orig_ds = pickle.load(orig_file)
    orig_ds.sort(key=lambda orig_ds: orig_ds[0])

    if type(def_path) == 'str':
        def_path = [def_path]

    q1_comp = np.random.choice([count for count, samp in enumerate(orig_ds) if samp[2] == 'Q1'], 2, replace=False)
    q2_comp = np.random.choice([count for count, samp in enumerate(orig_ds) if samp[2] == 'Q2'], 2, replace=False)
    q3_comp = np.random.choice([count for count, samp in enumerate(orig_ds) if samp[2] == 'Q3'], 2, replace=False)
    q4_comp = np.random.choice([count for count, samp in enumerate(orig_ds) if samp[2] == 'Q4'], 2, replace=False)

    for pt in def_path:
        with open(pt + '_samples_16kHz' + '.pickle', 'rb') as def_file:
            def_ds = pickle.load(def_file)
        dir_name = pt + '_for_comparison/'
        try:
            os.makedirs(dir_name)
        except FileExistsError:
            pass

        print('Working on {}...'.format(pt))
        for i1 in q1_comp:
            soundfile.write(dir_name + orig_ds[i1][0] + '.wav', orig_ds[i1][1], samplerate=sr)
            soundfile.write(dir_name + orig_ds[i1][0] + '_def.wav', def_ds[i1], samplerate=sr)
        for i2 in q2_comp:
            soundfile.write(dir_name + orig_ds[i2][0] + '.wav', orig_ds[i2][1], samplerate=sr)
            soundfile.write(dir_name + orig_ds[i2][0] + '_def.wav', def_ds[i2], samplerate=sr)
        for i3 in q3_comp:
            soundfile.write(dir_name + orig_ds[i3][0] + '.wav', orig_ds[i3][1], samplerate=sr)
            soundfile.write(dir_name + orig_ds[i3][0] + '_def.wav', def_ds[i3], samplerate=sr)
        for i4 in q4_comp:
            soundfile.write(dir_name + orig_ds[i4][0] + '.wav', orig_ds[i4][1], samplerate=sr)
            soundfile.write(dir_name + orig_ds[i4][0] + '_def.wav', def_ds[i4], samplerate=sr)

    print('Done saving sound files for {} deformation!'.format(def_path))


'''
def see_augmentations(samples, tranforms):
    with open('time_freq_masked_samples_16kHz.pickle', 'rb') as tmf:
        time_masked = pickle.load(tmf)
    # with open('frequency_masked_samples_16kHz.pickle', 'rb') as fmf:
    # frequency_masked = pickle.load(fmf)
    # with open('seven_banded_samples_16kHz.pickle', 'rb') as sbf:
    # seven_banded = pickle.load(sbf)
    # with open('tanh_disted_samples_16kHz.pickle', 'rb') as tdf:
    # tanh_disted = pickle.load(tdf)

    choice = np.random.choice(range(len(samples)), 1)[0]

    fig, axs = pyplot.subplots(2, 1)
    axs[0].imshow(np.rot90(to_melspectrogram(samples[choice])), cmap='inferno')
    axs[1].imshow(np.rot90(to_melspectrogram(time_masked[choice])), cmap='inferno')
    # axs[2].imshow(np.rot90(to_melspectrogram(frequency_masked[choice])), cmap='inferno')
    # axs[3].imshow(np.rot90(to_melspectrogram(seven_banded[choice])), cmap='inferno')
    # axs[4].imshow(np.rot90(to_melspectrogram(tanh_disted[choice])), cmap='inferno')
    pyplot.savefig('compare_mel_spect_' + tranforms + '.png')
'''
...
def run_f1score(X, labels, epochs, batch_size, learning_rate, deforms, resume_training=False):
    ...
    x_for_fold = np.array([1] * number_of_samples)
    y_for_fold = np.array([1] * number_of_samples)

    folds = list(kfold.split(x_for_fold, y_for_fold))
    ...
    for train, test in tqdm(folds):
        ...
        for i_train in train:
            ...
            for j in range(to_add):
                x_train.append(X[i_train + j])
                y_train.append(labels[i_train + j])
        for i_test in test:
            x_test.append(X[i_test * to_add])
            y_test.append(labels[i_test * to_add])
        ...
        train_gen = Generator_4QAED(x_train, y_train, batch_size)
        test_gen = Generator_4QAED(x_test, y_test, batch_size)
...
def my_gridsearch_simple(x, Y, deforms_names, num_deforms):
    ...
    for bs in range(len(matrix_of_params[0])):
        for ep in range(len(matrix_of_params[1])):
            for lr in range(len(matrix_of_params[2])):
                ...
                file_name = 'history_' + deforms_names + 'bs_' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1s_name = 'f1_score_simple_' + deforms_names + 'bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1e_name = 'f1_each_simple_' + deforms_names + 'bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                conf_name = 'conf_mat_simple_' + deforms_names + 'bs_' +str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                scores_name = 'scores_simple_' + deforms_names + 'bs_' + str(matrix_of_params[0][bs]) + \
                              '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                time_name = 'time_simple_' + deforms_names + 'bs_' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
...
def main_augmentations(file_with_labels):
    with open(file_with_labels + '.pickle', 'rb') as sounds_file:
        sounds = pickle.load(sounds_file)

    sounds.sort(key=lambda sounds: sounds[0])
    waveforms = np.array([temp[1] for temp in sounds])

    print('For augmentation.')
    print(waveforms.shape)
    print("Preprocessamento completo!")

    start = time.time()

    augmentate_data(waveforms)
    #see_augmentations(waveforms)
...
def main_run_augm(file_with_mel, file_with_labels, deforms, deforms_names):
    ...
    X = df_spect.reshape(df_spect.shape[0], df_spect.shape[1], df_spect.shape[2], 1)
    ...
    if len(deforms) > 0:
        ...
        for deform in deforms:
            melspect_dataset = 'melspect_' + deform + '_samples_16kHz'
            with open(melspect_dataset + '.pickle', 'rb') as deform_file:
                curr_deform = pickle.load(deform_file)
                all_deforms.append(curr_deform.reshape(
                    curr_deform.shape[0], curr_deform.shape[1], curr_deform.shape[2], 1))

        x_with_deforms, y_with_deforms = [], []
        for i in range(len(X)):
            x_with_deforms.append(X[i])
            y_with_deforms.append(Y_total[i])
            for j in range(len(deforms)):
                x_with_deforms.append(all_deforms[j][i])
                y_with_deforms.append(Y_total[i])
    else:
        print('No deformations chosen!')
        print('Please add deformations file names in deforms.')
        raise KeyError
    print("Preprocessamento completo!")

    print('Length of samples: {}'.format(len(x_with_deforms)))
    my_gridsearch_simple(x_with_deforms, y_with_deforms, deforms_names, len(deforms))
    print('Done')
...
if __name__ == "__main__":
    ...
    for trans in transforms:
        waveform_dataset = trans + '_samples_16kHz'
        melspect_dataset = 'melspect_' + waveform_dataset

    # Uncomment if you need to generate the augmented samples
    #create_melspect_dataset(waveform_dataset, melspect_dataset)

    main_run_augm(file_with_mel, file_with_samples, transforms, trans_names)
