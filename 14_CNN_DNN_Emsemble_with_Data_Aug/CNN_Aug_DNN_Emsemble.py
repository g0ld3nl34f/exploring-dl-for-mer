...
# For training purposes, DO NOT change
fold_acum = 1
...
# Name of path to the manually extracted features
path_handcrafted = 'handcrafted_features/decorrelated_1714_new_pub_complete_targets.csv'
# Path to saved DNN models for emsemble
path_to_dnn_models = curr_path + '08_DNN/best_DNN_All_Features_Models/'
# Number of features to be fed to the DNN portion of the network
num_features = 1704
...
def load_data_allfeatures():
    # load .csv with top 900 (decorrelated)
    data_all = pd.read_csv(
        # TODO: Change this path
        curr_path + ptd + path_handcrafted,
        sep=',', header=0)
    data_all.sort_values(by=['SongID'])
    # TODO: Change this path
    data = pd.read_csv('/home/pedrolouro/experiments-msc/New_Public_DB_Data/handcrafted_features/decorrelated_1714_new_pub_complete.csv', sep=',', header=0)
    data.sort_values(by=['IDSong'])

    # Forgot to remove index at 1, data starts at 2
    X = data.iloc[:, 2:]  # cabeçalho, 1a coluna -> nome, label no fim
    Y = data_all.iloc[:, -1]

    # normalizar dataset
    X_with_bad = preprocessing.scale(X.to_numpy().astype(np.float32))

    # Due to NaN values, columns [752 753 754 755 756 757 758 759 760 761] were initially dropped.
    # All NaN values should be changed to 0 to follow the original approach with Legacy MERGE.
    # Either keep the file as it is to accurately replicate the reported experiment
    # or make this change to make it consistent with the original approach.
    bad_feats = []

    for samp in X_with_bad:
        for count_feat, feat in enumerate(samp):
            if np.isnan(feat):
                bad_feats.append(count_feat)

    feats_to_remove = np.unique(np.array(bad_feats)).tolist()
    X = X.drop(X.columns[feats_to_remove], axis=1)

    X = preprocessing.scale(X.to_numpy().astype(np.float32))
    # Should be False, or training the model is impossible
...
###############################################
###################################
### Emsemble model -> CNN + DNN ###
###################################
def create_DNN_best_freeze():
    # DNN - load and freeze weights
    dnn = Sequential()
    input_dim = num_features
    dnn.add(BatchNormalization(input_dim=input_dim, name='batch_1_dnn'))
    dnn.add(Dropout(0.2, name='drop_1_dnn'))
    dnn.add(Dense(500, activation='relu', name='dense_1_dnn'))
    # dnn.add(Dense(150, activation='relu'))
    dnn.add(Dense(300, activation='relu', name='dense_2_dnn'))
    dnn.add(Dense(100, activation='relu', name='dense_3_dnn'))
    # dnn.add(BatchNormalization())
    dnn.add(Dense(4, activation='softmax', name='dense_4_dnn'))
    # FIXME: not loading to test
    dnn.load_weights(
        path_to_dnn_models + 'dnn_best_' + str(fold_acum) + '.h5')
    dnn.trainable = False
    return dnn


def create_CNN_best():
    # DNN com top 100 features + CNN (k2c2) com mel spectrogram
    # standard
    ...
    # model_k2c2.add(BatchNormalization(input_shape=(width_time, 128, 1)))
    model_k2c2.add(Conv2D(16, filters,  strides=(1, 1), input_shape=(width_time, 128, 1), activation='relu', data_format='channels_last',
                          kernel_initializer='he_normal'))
    model_k2c2.add(MaxPooling2D(pool_size=(2, 2), padding='same'))  # default poolsize = (2, 2), stride = (2,2)
    model_k2c2.add(BatchNormalization())
    model_k2c2.add(Dropout(0.1))
    # 10 filters, 5x5 receptive field, 1 stride, 60x644x1 input, 56x640 feature map
    model_k2c2.add(Conv2D(16, filters, strides=(1, 1), activation='relu', data_format='channels_last',
                          kernel_initializer='he_normal'))
    model_k2c2.add(MaxPooling2D(pool_size=(2, 2), padding='same'))  # default poolsize = (2, 2), stride = (2,2)
    model_k2c2.add(BatchNormalization())
    model_k2c2.add(Dropout(0.1))
    # 15 filters, 5x5 receptive field, 1 stride, 28x320x1 input, 24x316 feature map
    model_k2c2.add(Conv2D(16, filters, strides=(1, 1), activation='relu', data_format='channels_last',
                          kernel_initializer='he_normal'))
    model_k2c2.add(MaxPooling2D(pool_size=(3, 2), padding='same'))  # default poolsize = (2, 2), stride = (2,2)
    model_k2c2.add(BatchNormalization())
    model_k2c2.add(Dropout(0.1))
    model_k2c2.add(Conv2D(16, filters, strides=(1, 1), activation='relu', data_format='channels_last',
                          kernel_initializer='he_normal'))
    ...
    model_k2c2.add(Dropout(0.3))
    model_k2c2.add(Dense(300, activation='relu'))
    # model_k2c2.add(Dense(50, activation='relu'))
    # model_k2c2.add(Dense(100, activation='relu'))
    model_k2c2.add(Dense(num_classes, activation='softmax'))
    # cnn.add(Dense(300, activation='relu', name='dense_2_cnn'))
    # cnn.add(Dense(100, activation='relu', name='dense_3_cnn'))
    # cnn.add(Dense(4, activation='softmax', name='dense_4_cnn'))
    # load weights
    # model_k2c2.load_weights('CNN_best/cnn_best_' + str(fold_acum) + '.h5')

    # model_k2c2.trainable = False
    return model_k2c2


def compile_model(lr=0.01):
    dnn = create_DNN_best_freeze()
    cnn = create_CNN_best()
    # cnn.load_weights('initial_weights_hybrid_cnn.h5')

    model_comb = Concatenate()([dnn.layers[-2].output, cnn.layers[-2].output])
    model_comb = BatchNormalization(name='batch_0_final')(model_comb)
    # model_comb = Dropout(0.2, name='dropout_final_1')(model_comb)
    model_comb = Dense(200, activation='relu', name='dense_1_output', kernel_initializer='he_normal')(model_comb)
    # model_comb = BatchNormalization(name='batch_1_final')(model_comb)
    model_comb = Dense(100, activation='relu', name='dense_2_output', kernel_initializer='he_normal')(model_comb)
    # model_comb = BatchNormalization(name='batch_1_final')(model_comb)
    # model_comb = BatchNormalization(name='batch_2_final')(model_comb)
    model_comb = Dense(4, activation='softmax', name='dense_3_output', kernel_initializer='he_normal')(model_comb)
    # model_comb.summary()
    model_comb = Model(inputs=[dnn.input, cnn.input], outputs=model_comb)

    # compile model
    opt = SGD(learning_rate=lr)
    loss_alg = CategoricalCrossentropy()
    model_comb.compile(loss=loss_alg, optimizer=opt, metrics=['accuracy'])
    model_comb.summary()
...
def run_f1score(X_cnn, X_dnn, labels, epochs, batch_size, learning_rate, resume_training=False):
    ...
    # load folds because of the weight freeze
    with open("folds_dnn.pickle", "rb") as folds_file:
        folds = pickle.load(folds_file)
    index_best = 0
    ...
    for train, test in tqdm(folds):
        ...
        model = compile_model(lr=learning_rate)

        fold_acum += 1

        # train and save to log file
        history = model.fit(x=[X_dnn[train], X_cnn[train]], y=labels[train],
                            validation_data=([X_dnn[test], X_cnn[test]], labels[test]),
        ...
        if save_models:
            model.save("comb_best_models/dnn_cnn_" + str(epochs) + "_" + str(batch_size) + "_" + str(acum) + ".h5")

        ## Testing portion ##
        print('Testing ' + str(acum) + '...')
        # Evaluate the model
        score_temp = model.evaluate(x=[X_dnn[test], X_cnn[test]], y=labels[test], verbose=0)
        print(score_temp)
        scores_all.append(score_temp)

        # Predict test fold
        ypred_all = model.predict(x=[X_dnn[test], X_cnn[test]])
...
def my_gridsearch_simple(x, x_dnn, Y):
        ...
        for ep in range(len(matrix_of_params[1])):
            for lr in range(len(matrix_of_params[2])):
                ...
                if curr_candidate in resume_params_sgd:
                    ...
                    hist, f1s, f1e, conf, scores = run_f1score(x, x_dnn, Y,
                ...
                file_name = 'history_hybrid_cnn_dnn_bs_' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1s_name = 'f1_score_hybrid_cnn_dnn_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1e_name = 'f1_each_hybrid_cnn_dnn_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                conf_name = 'conf_mat_hybrid_cnn_dnn_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                scores_name = 'scores_hybrid_cnn_dnn_bs' + str(matrix_of_params[0][bs]) + \
                              '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                time_name = 'time_hybrid_cnn_dnn_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
...
def main(file_with_mel, file_with_labels):
    ...
    with open(file_with_labels + '.pickle', 'rb') as sounds_file:
        sounds = pickle.load(sounds_file)
    features, labels_for_features = load_data_allfeatures()
    ...
    # Check if labels are paired correctly
    comparison = labels == labels_for_features
    check = comparison.all()
    if check:
        print("\nLabels check out!\n")
    else:
        print("\nSomething went wrong with the labels!\n")
        return
    ...
    my_gridsearch_simple(X, features, Y_total)
