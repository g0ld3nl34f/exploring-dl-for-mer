...
aug_file = 'new_pub_complete_dataset_16kHz_melspect_norm_aug'
aug_targets = aug_file + '_target'
...
def generate_aug_all(file2save):
    # vv FIXME: Change for other dataset vv
    target = pd.read_csv(curr_path + ptd + csv_name, sep=',', header=0)
    target = target.set_index('Song')
    # create class object for augmentation functions
    aug = Augmented()

    # This max len only applies to sr=16000
    max_len = 482000
    sampling_rate = 16000
    files = [f for f in listdir(location) if isfile(join(location, f)) if '.wav' in f]
    sounds = []
    file_name = ['', '_1_time_shifted', '_2_pitch_shifted', '_3_time_stretched', '_4_db_shifted']
    ...
    for fname in tqdm(files):
        ...
        y, _ = librosa.load(wav_file_location, sr=sampling_rate)
        # returns original sample + 4 different
        if len(y) > max_len:
            print('Cutting {}...'.format(f_just_name))
            to_cut = len(y) - max_len
            y = y[to_cut:]
        augmented_sound = aug.augment(y, sampling_rate)

        for acum, y_temp in enumerate(augmented_sound):
            sounds.append([f_just_name[:-4] + file_name[acum], y_temp, target.loc[f_just_name[:-4], 'Quadrant']])

    sounds.sort(key=lambda sounds: sounds[0])
    print("\nAugmented and sorted!")

    # generate melspectrogram
    print("Generating melspectrogram...")

    # the sample with db_shift already is a mel spectrogram
    spect_all = []
    sounds = np.array(sounds)
    for acum, temp in enumerate(sounds[:, 1]):
        if (acum + 1) % 5 == 0 and acum != 0:
            spect_all.append(temp)
        else:
            spect_all.append(librosa.power_to_db(librosa.feature.melspectrogram(temp)))

    width_time = 942
    # padding line column and rotation
    # [samples][width][height][channels]
    # padding to form 942 (width_time)
    ...
    # save to file
    with open(file2save + ".pickle", "wb") as sounds_file:
        pickle.dump(df_spect, sounds_file, protocol=4)
    with open(file2save + '_target' + ".pickle", "wb") as target_file:
        pickle.dump(targets, target_file, protocol=4)

    print("Done!\nSaved melspectrograms to " + file2save + ".pickle" + " and targets to " + file2save + "_target.pickle")
    return
...
#############################
def create_DNN_best_freeze(fold_acum):
    ...
    dnn.load_weights(
        'DNN_best/dnn_best_' + str(fold_acum) + '.h5')
    dnn.trainable = False
    return dnn
...
def create_CNN_best():
    ...
    clear_session()  # libertar memoria
    gc.collect()
    # DNN with all features + CNN with Mel-spectrogram
    # standard
...
def run_f1score(X, labels, epochs, batch_size, learning_rate, resume_training=False):
    ...
    for train, test in tqdm(folds):
        ...
        for i in train:
            i *= 5  # Number of real sample + aug samples from real

            x_train.extend(X[i:i + 5])
            y_train.extend(labels[i:i + 5])

        for j in test:
            j *= 5  # Get real sample corresponding to value
        ...
        model.save("CNN_best_models/cnn_best" + "_" + str(acum) + '.h5')
        ...
        # Evaluate the model
        score_temp = model.evaluate(x=x_test, y=y_test, verbose=0)
        print(score_temp)
        scores_all.append(score_temp)

        # Predict test fold
        ypred_all = model.predict(x=x_test)

        # Calculate Confusion Matrix
        print(confusion_matrix(np.asarray(y_test), np.asarray(ypred_all).argmax(axis=1)))
        conf_temp = confusion_matrix(np.asarray(y_test), np.asarray(ypred_all).argmax(axis=1))
        confusion_matrix_global = confusion_matrix_global + conf_temp

        # Predict for F1 Score
        f1_temp = f1_score(np.asarray(y_test), np.asarray(ypred_all).argmax(axis=1), average='macro')
        f1_each.append(f1_score(np.asarray(y_test), np.asarray(ypred_all).argmax(axis=1), average=None))
    ...
    for bs in range(len(matrix_of_params[0])):
        for ep in range(len(matrix_of_params[1])):
            for lr in range(len(matrix_of_params[2])):
                ...
                if curr_candidate in resume_params_adam:
                ...
                elif curr_candidate in ran_params_adam:
                ...
                file_name = 'history_simple_bs_' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1s_name = 'f1_score_simple_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                f1e_name = 'f1_each_simple_bs_' + str(matrix_of_params[0][bs]) + \
                           '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                conf_name = 'conf_mat_simple_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                scores_name = 'scores_simple_bs' + str(matrix_of_params[0][bs]) + \
                              '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
                time_name = 'time_simple_bs' + str(matrix_of_params[0][bs]) + \
                            '_ep_' + str(matrix_of_params[1][ep]) + '_lr_' + str(matrix_of_params[2][lr])
...
def main(file_with_mel, file_with_labels):
    ...
    with open(file_with_labels + '.pickle', 'rb') as sounds_file:
        labels = pickle.load(sounds_file)
    ...
    my_gridsearch_simple(X, Y_total)
...
if __name__ == "__main__":
    ...
    file_with_augments = curr_path + ptd + aug_file
    file_with_augs_targets = curr_path + ptd + aug_targets
    ...
    main(file_with_augments, file_with_augs_targets)
